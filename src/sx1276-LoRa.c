...
tLoRaSettings LoRaSettings =
{
    470300000,      /* 载波频率 */
    17,               /* 发射功率 */
    7,                /* 带宽  [0: 7.8kHz, 1: 10.4 kHz, 2: 15.6 kHz, 3: 20.8 kHz, 4: 31.2 kHz,
...
{
	printf("LoRa Init doing........\n");
    RFLRState = RFLR_STATE_IDLE; //default idle

    SX1276LoRaSetDefaults( ); // read SX1276LR version
    printf("LoRa version default is 0x12,now read version = %x\n",SX1276LR->RegVersion);
    SX1276ReadBuffer( REG_LR_OPMODE, SX1276Regs + 1, 0x70 - 1 );
    
    SX1276LR->RegLna = RFLR_LNA_GAIN_G1;//set lna is Maximum gain :001 低噪声放大器
...
void SX1276LoRaSetDefaults( void )
{
    ...
    SX1276Read( REG_LR_VERSION, &SX1276LR->RegVersion ); // read SX1276LR verison
}


/*
 * LoRa mode
 * make suer mode and write register
 * 0x00 sleep mode
 * 0x01 standby mode
 */


void SX1276LoRaSetOpMode( uint8_t opMode )//opMOde  = 0x00
{
    static uint8_t opModePrev = RFLR_OPMODE_STANDBY;
    static bool antennaSwitchTxOnPrev = true;
    bool antennaSwitchTxOn = false;

    opModePrev = SX1276LR->RegOpMode & ~RFLR_OPMODE_MASK; //opModePrev = 0x01

    if( opMode != opModePrev )
    {
        if( opMode == RFLR_OPMODE_TRANSMITTER ) //if mode is TX
        {
            antennaSwitchTxOn = true; //antenna switch tx on
        }
        else
        {
...
//            RXTX( antennaSwitchTxOn ); // Antenna switch control
        ...
        SX1276LR->RegOpMode = ( SX1276LR->RegOpMode & RFLR_OPMODE_MASK ) | opMode; //standby mode
...
void LoRaRxDataRead (uint8_t *pbuf, uint8_t *size )
{
	...
	if( LoRaSettings.ImplicitHeaderOn == true ) {
		...
		SX1276Read( REG_LR_NBRXBYTES, size );
//		RxPacketSize = SX1276LR->RegNbRxBytes;
		SX1276LR->RegFifoAddrPtr = SX1276LR->RegFifoRxCurrentAddr;
		SX1276Write( REG_LR_FIFOADDRPTR, SX1276LR->RegFifoAddrPtr );
		printf("read FIFO\n");
		uint16_t fifo_size = (uint16_t)(*size);
		SX1276ReadFifo( pbuf, fifo_size);
		*size = (uint8_t)fifo_size;
	}
}	
...
//    	Freq = 500300000;
    ...
    else
    {
    	...
    	Freq = 505300000;
    	SX1276FreqSet(Freq);
    	SX1276LoRaSetSpreadingFactor((uint8_t)12);
    }


//	SX1276LoRaSetSpreadingFactor((uint8_t)9);
//	SX1276SetOpMode(RFLR_OPMODE_STANDBY);
//    SX1276LoRaSetPreambleLength(65530);
	uint8_t iq;
	SX1276Read(AM_SX1276_LORA_REG_INVERTIQ,&iq);

	SX1276Write(AM_SX1276_LORA_REG_INVERTIQ,((iq &
                     AM_SX1276_LORA_RF_INVERTIQ_TX_MASK &
                     AM_SX1276_LORA_RF_INVERTIQ_RX_MASK) |
                    AM_SX1276_LORA_RF_INVERTIQ_RX_OFF |
                    AM_SX1276_LORA_RF_INVERTIQ_TX_ON));
	SX1276Write(AM_SX1276_LORA_REG_INVERTIQ2, AM_SX1276_LORA_RF_INVERTIQ2_ON);
	...
	// Initializes the payload size
	if(crcflag ==1)
		SX1276LR->RegPayloadLength = size + 4; //TxPacketSize;
	else
